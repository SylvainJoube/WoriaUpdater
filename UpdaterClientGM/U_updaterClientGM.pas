...
const G_publicPort_rescue = 443;   // si le port classique ne répond pas

// voir TGestionUpdater.Init pour la gestion des différentes adresses
...
type TGestionUpdater_netAddress = record // une configuration à tester avant d'afficher le message "impossible de se connecter"
  Ip : string; // ip destination
  Port : word; // port destination
  StrToDraw : string; // message à afficher pour dire au joueur de patienter
end;
...
type TGestionUpdater = class
  ...
  public
    ...
    A1NetAddressToTry : array of TGestionUpdater_netAddress; // Liste de toutes les configurations à essayer avant de dire qu'il est impossible de se connecter
    NetAddress_currentTryIndex : cardinal; // = 0, je commence par la première configuration possible
    ...
    procedure AddAdressToTry(ip : string; port : word; putFirst : boolean = false; afficherMessageStr : string = ''); // ajout d'une adresse à essayer
    ...
    function HasNextAdressToTry : boolean; // true s'il reste une adresse à essayer, false sinon
...
function dGetCurrentString(arg_valueIndex : double) : double; cdecl;
begin
  if arg_valueIndex=0 then begin // L'indexation des caractères du string commence à 1.
...
begin
  Result:=1;
  if arg_index=0 then begin // L'indexation des caractères du string commence à 1.
...
begin
  ...
  // -> Faire la désinstallation avec un thread.
  UninstallLog_message:='Désinstallation...';
  ...
  UninstallLog_message:='Désinstallation terminée.';
  G_updaterState:=0;
end;
...
begin
  ...
  // Ajout des différentes adresse à tester :
  self.AddAdressToTry(G_publicIp, G_publicPort, false, 'serveur principal (port par défaut)');
  self.AddAdressToTry(G_publicIp, G_publicPort_rescue, false, 'serveur principal (port alternatif)');
  self.AddAdressToTry(G_publicIp_rescue, G_publicPort, false, 'serveur de secours (port par défaut)');
...
begin            //System.Win.ScktComp
  ...
  //ShowMessage('Connexion à ip ='+ip+' port = '+inttostr(port)+' addrNameGraphic = ' + addrNameGraphic);
  ...
  strPortInfo := ''; // info relative au port utilisé
  //if (port = G_publicPort) then strPortInfo := ' (port par défaut)';
  ...
  if addrNameGraphic = '' then begin
    ...
    strInfo := strInfo +' à ' + addrNameGraphic + '...';
  end;
...
begin
  ...
  UpdateButtonCaption(Messages.StrActionActuelle, messageStr);
  Result := success; // visiblement non utilisé
end;
...
begin
  ...
  NetAddress_currentTryIndex := 0; // réinitialisation de l'index, comme une connexion a bien été établie (je pourrais aussi utiliser NetAddress_currentTryIndex := max(0, integer(NetAddress_currentTryIndex) - 1)
  b1:=readubyte;
  b2:=readubyte;
  ...
  if Reseau_currentStep=1 then begin
    ...
    if (b1=1) and (b2=1) then begin // Reseau_currentStep = 1
      ...
      sendbuffer(client);
      SendAllBuffers;
      G_updaterState:=2; // attente de la réception de la liste de fichiers
      UpdateButtonCaption(Messages.StrActionActuelle, 'Attente de la réception de la liste des fichiers...');
      G_updaterMessage:='Attente de la réception de la liste des fichiers...';
  ...
  if Reseau_currentStep=2 then begin
    ...
    G_updaterMessage:='Création du dossier du jeu...';
    //if not PrivateTest then
    ...
    if not DirectoryExists(gameDir) then begin // bug : impossible de créer le dossier du jeu.
      messageStr:='Echec de l''installation : impossible de créer le dossier du jeu.'+chr(10)+'-> Exécutez ce programme en administrateur+'+chr(10)+'-> Désactivez votre antivirus';
      ShowMessage(messageStr);
      G_updaterMessage:=messageStr;
      G_updaterState:=0;
    ...
    G_updaterMessage:='Téléchargement des fichiers...';
    UpdateButtonCaption(Messages.StrActionActuelle, 'Téléchargement des fichiers...');

    bRes:=Update_receiveFileList(client, gameDir, true);
    NeededFiles_receivedSizeTotal := 0; // réinitialisation du nombre total d'octets téléchargés
    IsDownloading:=bRes;

    //UpdateButtonEnabled(Messages.ButtonPlay, not IsDownloading);
    if IsDownloading then G_updaterMessage:='Mise à jour...'; //UpdateButtonCaption(Messages.ButtonPlay, 'Mise à jour...');
  ...
  if Reseau_currentStep=5 then begin // téléchargement des fichiers du jeu
    ...
    while true {client.GetMessage} do begin
      ...
      if (b1=253) then begin
        ...
        G_updaterMessage:='L''updater est en maintenance, veuillez réessayer plus tard. Désolé ><''';
        UpdateButtonCaption(Messages.StrActionActuelle, 'L''updater est en maintenance, veuillez réessayer plus tard. Désolé ><''');
        G_updaterState:=0;
        IsDownloading:=false;
        JeuAJour(false);
        Reseau_currentStep:=0;
...
procedure TGestionUpdater.AddAdressToTry(ip : string; port : word; putFirst : boolean = false; afficherMessageStr : string = ''); // ajout d'une adresse à essayer
...
begin
  ...
  if putFirst then begin
    if newLen <> 0 then // vérif inutile
...
begin
  ...
  Result := true; // encore une adresse à essayer
...
function TGestionUpdater.HasNextAdressToTry : boolean; // true s'il reste une adresse à essayer, false sinon
begin
  if integer(NetAddress_currentTryIndex) >= length(A1NetAddressToTry) then begin // plus aucune adresse à essayer
    Result := false;
    exit;
  end;
  Result := true;
